 ...
 * Sums actual, predicted, lower/upper bounds per ISO week.
 */
function aggregateWeekly(data) {
    ...
    data.forEach(d => {
        ...
        if (!weeks[weekKey]) {
            weeks[weekKey] = { date: weekKey, actual: 0, predicted: 0, lower_bound: 0, upper_bound: 0, hasActual: false, hasPredicted: false, dayCount: 0 };
        }
        ...
        if (d.lower_bound !== undefined) weeks[weekKey].lower_bound += d.lower_bound;
        if (d.upper_bound !== undefined) weeks[weekKey].upper_bound += d.upper_bound;
    ...
    return Object.values(weeks)
        ...
        .map(w => ({
            ...
            ...(w.hasPredicted ? { lower_bound: Math.round(w.lower_bound), upper_bound: Math.round(w.upper_bound) } : {}),
        }));
}

const ForecastChart = ({ data, showConfidenceBand = true }) => {
    const [viewMode, setViewMode] = useState('weekly'); // 'daily' or 'weekly'
    
    const chartData = useMemo(() => {
        // Split into historical (has 'actual') and forecast (has 'predicted')
        ...
        // Trim historical to keep chart focused — avoids old outlier spikes dominating Y-axis
        const maxHistDays = viewMode === 'weekly' ? 28 : 14;
        const trimmedHist = historicalPoints.slice(-maxHistDays);
        const trimmedData = [...trimmedHist, ...forecastPoints];
        
        const source = viewMode === 'weekly' ? aggregateWeekly(trimmedData) : trimmedData;
        const interval = viewMode === 'weekly' ? 1 : 7;
        return source.map((d, i) => ({
            ...d,
            predictedLabel: (i % interval === 0 || i === source.length - 1) && d.predicted !== undefined 
                ? Math.round(d.predicted) 
                : null,
            actualLabel: (i % interval === 0 || i === source.length - 1) && d.actual !== undefined 
                ? Math.round(d.actual) 
                : null,
        }));
    }, [data, viewMode]);
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '0.75rem', gap: '4px' }}>
                <button
                    ...
                    style={{
                        ...
                        border: 'none',
                        cursor: 'pointer',
                        background: viewMode === 'daily' ? '#0D9488' : '#f1f5f9',
                        color: viewMode === 'daily' ? '#fff' : '#64748b',
                        transition: 'all 0.2s'
                    }}
                >
                    Daily
                ...
                <button
                    ...
                    style={{
                        ...
                        border: 'none',
                        cursor: 'pointer',
                        background: viewMode === 'weekly' ? '#0D9488' : '#f1f5f9',
